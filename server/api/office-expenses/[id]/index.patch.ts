...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        const currentExpenseResult = await neonClient`SELECT * FROM office_expenses WHERE id = ${id}`;
        const currentExpense = currentExpenseResult[0];
        ...
        const queryArr = [neonClient.query(replaceQuestionMarks(sql), params)];
        ...
        if (isNowPaid && isRecurrent) {
            ...
            queryArr.push(neonClient`
                INSERT INTO office_expenses (description, amount, due_date, status, is_recurrent)
                    VALUES (${description || currentExpense.description}, ${amount !== undefined ? amount : currentExpense.amount}, ${nextDueDateStr}, 'Pendente', true)
            `)
        }

        await neonClient.transaction(queryArr);
